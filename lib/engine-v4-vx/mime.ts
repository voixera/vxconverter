...
export class MimeDetector {
  private static EXT_MIME_MAP: Record<string, { mime: string; kind: "video" | "audio" | "stream" | "image"; playable: boolean }> = {
    ...
    mov: { mime: "video/quicktime", kind: "video", playable: false }, // Some browsers fail on quicktime container
  ...
  /**
   * Determine exact MIME, extension, kind and native browser playability
   */
  public static resolve(url: string, declaredMime?: string | null): MimeInfo {
    ...
    // If declared MIME is a known media MIME
