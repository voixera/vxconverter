...
function candidate(url: string, source: string, mime = "video/mp4"): Candidate {
  const extension =
    mime.includes("mpegurl") || /\.m3u8(?:$|\?)/i.test(url)
      ? "m3u8"
      : mime.includes("audio")
        ? "mp3"
        : "mp4";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const pattern =
      /<(?:video|audio|source)[^>]+src=["']([^"']+)["'][^>]*>|<meta[^>]+(?:property|name)=["'](?:og:video|og:audio|twitter:player:stream)["'][^>]+content=["']([^"']+)["']/gi;
